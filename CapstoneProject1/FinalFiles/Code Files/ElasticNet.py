...
steps = [('scaler', StandardScaler()), ('elasticnet', ElasticNet(max_iter=100000))]
pipeline = Pipeline(steps)

parameters = {'elasticnet__l1_ratio':np.linspace(0,1,100)}
#parameters = {'l1_ratio':np.linspace(0,1,100)}

reg = GridSearchCV(pipeline, param_grid=parameters)
...
result.to_csv("submission_Rafael7.csv", index=False)